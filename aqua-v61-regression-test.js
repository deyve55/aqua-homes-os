...
function checkStaticFiles() {
  ...
  addCheck('v62V-B natural response integration namespace exists', /window\.AquaNaturalResponseIntegrationV62VB/.test(extension), { layer: 'natural-response-integration-v62vb', fileToFix: EXTENSION });
  addCheck('v62V-B natural response integration helpers exist', /function\s+applyAquaNaturalResponseToAssistantV62VB/.test(extension) && /function\s+buildAquaAssistantSurfaceCopyV62VB/.test(extension) && /function\s+renderAquaNaturalResponsePreviewV62VB/.test(extension), { layer: 'natural-response-integration-v62vb', fileToFix: EXTENSION });
  addCheck('v62V-B natural response integration report flags exist', /naturalResponseIntegrationExists/.test(extension) && /assistantSurfaceNaturalCopyWorks/.test(extension), { layer: 'natural-response-integration-v62vb', fileToFix: EXTENSION });
}
...
function runExtensionRegression() {
  try {
    ...
    addCheck('extension regression version is v62V-B', extensionReport.version === 'v62V-B', { layer: 'extension-regression', actual: extensionReport.version, fileToFix: EXTENSION });
    ...
    ['show aqua brain assistant','open aqua brain assistant','show assistant interface','show primary ai interface','show Home Depot receipts for Henderson','prepare those for accountant export','what needs approval','read it back','continue','manual controls','show quick routes','show automation tools','show safety tools','clear assistant context','show automation report','run regression qa','banana test'].forEach((command) => {
      const row = (extensionReport.results || []).find((result) => result.command === command);
