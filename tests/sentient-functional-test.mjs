...
test("the living Neural Link focuses one thought without isolating Aqua's other systems", async () => {
  ...
  assert.match(script, /class="neural-traveler neural-traveler-cyan"/);
...
test("Aqua's widget is a universal executive handoff and confirms direct calendar writes", async () => {
  ...
  assert.match(widgetVerifier, /for resize_attempt in \$\(seq 1 7\)/);
  assert.match(widgetVerifier, /edge_offset=\$\(\(-72 \+ \(resize_attempt - 1\) \* 24\)\)/);
  assert.match(widgetVerifier, /start_x=\$\(\(right \+ edge_offset\)\)/);
  assert.match(widgetVerifier, /end_x=\$\(\(start_x \+ resize_distance\)\)/);
  assert.match(widgetVerifier, /end_x > screen_width - 24/);
  assert.match(widgetVerifier, /start_y=\$\(\(bottom \+ edge_offset\)\)/);
  assert.match(widgetVerifier, /end_y=\$\(\(start_y \+ resize_distance\)\)/);
  assert.match(widgetVerifier, /end_y > screen_height - 24/);
  assert.doesNotMatch(widgetVerifier, /right \+ width \* \(60 \+ resize_attempt \* 10\) \/ 100/);
  assert.doesNotMatch(widgetVerifier, /bottom \+ height \* \(60 \+ resize_attempt \* 10\) \/ 100/);
...
test("responsive widget fills every supported host while preserving approved art and action zones", async () => {
  ...
  assert.match(threeByTwoLayout, /android:weightSum="6"/);
  assert.equal((threeByTwoLayout.match(/android:layout_weight="1"/g) || []).length >= 2, true);
  assert.match(threeByTwoLayout, /android:layout_weight="4"/);
